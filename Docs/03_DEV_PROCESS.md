...
- 后续任何 AI 读取本项目时,必须按完整 M0～M7 架构说明当前进度,不能只根据已经完成的 M6 或当前文件给出局部结论。
- 说明进度时必须同时区分总体目标、源码实现、构建结果、板级证据、未验证项和项目决定暂缓/跳过的内容,不得把“未实现”说成“已完成”。
- 对未实现模块必须说明它在完整项目中的结合方式,包括接口、数据流、任务所有权、硬件/协议入口、依赖和验收方式。例如 M7 低功耗应说明 `0x03AA` 睡眠请求如何由 ProtocolTask/ControlTask 交给 PowerManager,再协调 StorageTask flush/关文件、停止 ADC/DMA、等待 RS485 空闲、RTC/EXTI 唤醒、WFI 以及唤醒后的恢复和 `0x0381` 事件;当前只需明确这些代码入口和验证仍未完成。
- 当前项目范围已经在 M6 收口;M7 不纳入本次结项。以后若继续开发,从 M7 低功耗闭环和总验收开始,而不是把 M7 的设计描述当作现有功能。
