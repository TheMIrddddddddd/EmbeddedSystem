...
### 3.5 SPI + GD25Q16(外部 Flash)

- **原理**:SPI 同步串行总线,读写 GD25Q16 NOR Flash(2MB)
- **项目用途**:参数持久化存储(变比/阈值/ID/波特率/协议模式)与少量告警记录;APP 中仅由 StorageTask 承载的 PersistenceService 访问
- **关键技术点**:
...
- **原理**:内部 Flash 擦写控制器
- **项目用途**:① 参数区读写 ② Bootloader 固件搬运(读暂存区→写 App 区)
...
- **项目用途**:上电跳转 + RS485 在线 IAP + TF 卡离线升级 + 双槽元数据 + 备份回滚
...
- **可靠性约束**:RECEIVING/STAGED_VALID/INSTALLING/TRIAL_PENDING/CONFIRMED/ROLLBACK 状态可恢复;离线成功包与失败包使用版本+CRC 唯一文件名隔离;64KB Bootloader 发布映像限定 ≤60KB,保留至少 4KB 余量
...
- **当前所有权**:
  - GD25Q16/SPI/FlashKV:StorageTask 单一所有者,不向业务公开 `bsp_spi_lock/unlock`,无需跨任务 SPI 互斥锁
